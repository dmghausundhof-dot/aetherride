#!/usr/bin/env npx tsx
/**
 * Classify + tile + upload country/sheet ways overlays.
 *
 * Fetch lives in build-ways-from-geofabrik.sh (regional extracts only —
 * not germany-latest / france-latest / planet).
 *
 *   npx tsx scripts/routing/build-ways-overlay.ts --sheet nl --ingest-seq FILE
 *   npx tsx scripts/routing/build-ways-overlay.ts --sheet nl --tile
 *   npx tsx scripts/routing/build-ways-overlay.ts --sheet nl --skip-fetch --upload
 */
import fs from "fs";
...
const SHEETS_PATH = path.join(ROOT, "scripts/routing/ways-sheets.json");
const MIN_LEN_M = 40;
...
function loadSheet(id: string): Sheet {
  const data = JSON.parse(fs.readFileSync(SHEETS_PATH, "utf8")) as {
    sheets: Sheet[];
  };
  const hit = data.sheets.find((s) => s.id === id);
  if (!hit) throw new Error(`unknown ways sheet ${id}`);
  return hit;
}
...
function parseArgs(argv: string[]) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (argv[i] === "--sheet") out.sheet = argv[++i] ?? "dach";
  }
  return out;
}
...
function keepWay(tags: Record<string, string | undefined>) {
...
async function ingestSeq(
  ...
  for await (const line of rl) {
    ...
    for (const rawLine of lines as unknown[]) {
      ...
      const coords = clipLine(simplify(rawLine), bbox);
...
async function tileAndMaybeUpload(sheet: Sheet, upload: boolean) {
  const outDir = path.join(ROOT, "data/routing/dist/_basemap");
  const seqPath = path.join(outDir, `${sheet.file}.geojsonseq`);
  ...
  if (!fs.existsSync(seqPath) || fs.statSync(seqPath).size < 2000) {
    throw new Error(`${sheet.file}.geojsonseq too small`);
  }
  console.log(`==> tippecanoe z10–z13 ${sheet.file}`);
  ...
  if (magic.toString("utf8", 0, 7) !== "PMTiles") {
    throw new Error(`${sheet.file}.pmtiles missing PMTiles magic`);
  }
  const pmBytes = fs.statSync(pmtilesPath).size;
  console.log("pmtiles", pmBytes);
  if (pmBytes > 2 * 1024 * 1024 * 1024) {
    throw new Error(`${sheet.file}.pmtiles exceeds 2 GiB — refuse upload`);
  }
  const meta = {
    id: sheet.file,
    name: sheet.name,
...
async function main() {
  ...
  const seqPath = path.join(outDir, `${sheet.file}.geojsonseq`);
  if (args.ingestSeq) {
    await ingestSeq(args.ingestSeq, seqPath, sheet.bbox);
    return;
  }
  if (args.tile || args.upload) {
    await tileAndMaybeUpload(sheet, args.upload);
    return;
  }
  throw new Error(
    "use bash scripts/routing/build-ways-from-geofabrik.sh --sheet … (or --ingest-seq / --tile / --upload)"
  );
}
