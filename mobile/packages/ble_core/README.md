...
- **Bosch LDI (Spec V1.0, Android 12+ / iOS)**: Phone is BLE accessory. Advertise service solicitation `0000eb20-eaa2-…` on pair **and** ride reconnect, bike (firmware ≥19) connects as central, we subscribe to Live Data `eb21` and decode the public protobuf. Pairing: Flow → Komponenten → FlowLine. LDI-Werte werden mit CSC gemerged (SoC/Odo nicht vom Ticker gelöscht). Ride retries LDI if the bike is still waking. Never invent SoC.
